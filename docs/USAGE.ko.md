...
### 매크로 브라우저 없이 타이핑으로 넣기

- **편집기 자동완성** — `{` 입력 후 `md-share`(또는 별칭 `markdown`)까지 치고 Enter.
  리치 에디터 본문에 위키 마크업을 그냥 타이핑하면 텍스트로만 남습니다.
- **삽입 → 위키 마크업** (Ctrl+Shift+D) — 위키 마크업을 통째로 변환합니다.
  닫는 태그는 슬래시 없이 같은 이름입니다 (`{/md-share}` 아님):

  ```
  {md-share:attachment=report.md}

  {md-share:url=https://md-share.apps.example.com/d/abc123}

  {md-share}
  # 본문에 직접 쓰는 markdown
  {md-share}
  ```
- **Storage format** (REST API·자동화용) — 본문은 `ac:plain-text-body`로 감쌉니다:

  ```xml
  <ac:structured-macro ac:name="md-share">
    <ac:parameter ac:name="url">https://md-share.apps.example.com/d/abc123</ac:parameter>
  </ac:structured-macro>

  <ac:structured-macro ac:name="md-share">
    <ac:plain-text-body><![CDATA[# markdown 본문]]></ac:plain-text-body>
  </ac:structured-macro>
  ```
...
- GFM만 사용: 표, 체크리스트, 언어 태그 붙은 코드 블록. **raw HTML은 제거됩니다**
  (예외: `<br>`은 실제 줄바꿈으로 렌더링 — 표 셀 안에서 줄을 바꾸는 유일한 방법)
- mermaid 크기 요령:
  - 선형 흐름은 `flowchart LR`(가로) — `TD`는 세로로 길어집니다
  - 노드 라벨은 짧게, 수치는 옆의 표로
  - 10노드를 넘으면 자연스러운 경계에서 두 다이어그램으로 분할
  - 괄호·중괄호(`(){}[]`, `${...}`)가 들어가는 라벨은 `"..."`로 감싸기 —
    특히 엣지 라벨 `-->|${var} 참조|`는 따옴표 없이는 파스 에러가 납니다
    (`-->|"${var} 참조"|`로 작성)
